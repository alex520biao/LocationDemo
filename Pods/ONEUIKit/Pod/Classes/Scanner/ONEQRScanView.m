...
#define ONEScannerLocalizedStr(str) ONELocalizedStr(str, @"ONEUIKit-Scanner")
...
@interface ONEQRScanView ()<AVCaptureMetadataOutputObjectsDelegate>
...
- (void)configCameraAndStart{
    [ONEProgressHUD showOnView:self];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [ONEProgressHUD dismissForView:self];
        });
    });
}
...
+ (BOOL)statusCheck{
    if (![self isCameraAvailable]){
        [ONEAlertViewManager showAlertONEAlertView:^(ONEAlertViewModel *model) {
            model.message = ONEScannerLocalizedStr(@"设备无相机——设备无相机功能，无法进行扫描");
            model.buttonTitles = @[ONEScannerLocalizedStr(@"好")];
        } completion:nil];
        return NO;
    }
    ...
    if (![self isRearCameraAvailable] && ![self isFrontCameraAvailable]) {
        [ONEAlertViewManager showAlertONEAlertView:^(ONEAlertViewModel *model) {
            model.message = ONEScannerLocalizedStr(@"设备相机错误——无法启用相机，请检查");
            model.buttonTitles = @[ONEScannerLocalizedStr(@"好")];
        } completion:nil];
        return NO;
    }
    ...
    if (![self isCameraAuthStatusCorrect]) {
        [ONEAlertViewManager showAlertONEAlertView:^(ONEAlertViewModel *model) {
            model.message = ONEScannerLocalizedStr(@"未打开相机权限 ——请在“设置-隐私-相机”选项中，允许滴滴访问你的相机");
            model.buttonTitles = @[ONEScannerLocalizedStr(@"好")];
        } completion:nil];
        return NO;
    }
